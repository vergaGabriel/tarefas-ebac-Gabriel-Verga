package br.com.gverga.cadastro;

import br.com.gverga.cadastro.dao.ClienteMapDAO;
import br.com.gverga.cadastro.dao.IClienteDAO;
import br.com.gverga.cadastro.dao.ProdutoDAO;
import br.com.gverga.cadastro.dao.IProdutoDAO;
...
public class App {
    ...
    public static void main(String[] args) {
        String opcaoCadastro = JOptionPane.showInputDialog(null,
                "Digite 1 para Cliente ou 2 para Produto",
                "Opcao Cadastro", JOptionPane.INFORMATION_MESSAGE);

        while (!isOpcaoValida(opcaoCadastro)) {
            if("".equals(opcaoCadastro)) {
                sair();
            }
            opcaoCadastro = JOptionPane.showInputDialog(null,
                    "Opção invalida digite 1 para Cliente ou 2 para Produto",
                    "Opcao Cadastro", JOptionPane.INFORMATION_MESSAGE);
        }

        if (isCliente(opcaoCadastro)) {
            ...
            while (isOpcaoValida(opcao)){
                ...
                if (isOpcaoSair(opcao)) {
                    ...
                    String dados = JOptionPane.showInputDialog(null,
                            "Digite os dados do produto separados por virgula, conforme exemplo: Nome, Codigo, Preco",
                            "Cadastro", JOptionPane.INFORMATION_MESSAGE);
                    cadastrar(dados);
                } else if (isConsultar(opcao)) {
                    String dados = JOptionPane.showInputDialog(null,
                    ...
                    String codigo = JOptionPane.showInputDialog(null,
                            "Digite o Codigo do produto que deseja ser alterado: ",
                            "Alteração", JOptionPane.INFORMATION_MESSAGE);
                    ...
                    if (produto != null) {
                        String dados = JOptionPane.showInputDialog(null,
                                "Digite os dados novos do produto separados por virgula, conforme exemplo: Nome, Codigo, Preco: ",
                                "Alteração", JOptionPane.INFORMATION_MESSAGE);
                        alteracao(dados);
                    } else {
                        JOptionPane.showMessageDialog(null, "Produto não existe para alteração!", "Erro", JOptionPane.INFORMATION_MESSAGE);
                    }
    ...
    private static boolean isCliente(String opcaoCadastro) {
        if ("1".equals(opcaoCadastro)) {
            return true;
        }
        return false;
    }

    private static boolean isProduto(String opcaoProduto) {
        if ("2".equals(opcaoProduto)) {
            return true;
        }
        return false;
    }
